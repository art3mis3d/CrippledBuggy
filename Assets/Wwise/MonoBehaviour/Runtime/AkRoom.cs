...
public class AkRoom : AkTriggerHandler
{
	...
	private void _SetGeometryFromCollider(bool needSetGeometry)
	{
		...
		if (roomCollider.GetType() == typeof(UnityEngine.MeshCollider))
		{
			...
			if (needSetGeometry)
			{
				AkSurfaceReflector.SetGeometryFromMesh(meshCollider.sharedMesh, geometryID, false, false);
			}
		...
		else if (roomCollider.GetType() == typeof(UnityEngine.BoxCollider))
		{
			...
			if (needSetGeometry)
			{
				AkSurfaceReflector.SetGeometryFromMesh(mesh, geometryID, false, false);
			}
			AkSurfaceReflector.SetGeometryInstance(geometryID, geometryID, INVALID_ROOM_ID, tempGameObject.transform, false);
